...
import 'schema/administrator_record.dart';
...
export 'schema/administrator_record.dart';
...
/// Functions to query AdministratorRecords (as a Stream and as a Future).
Future<int> queryAdministratorRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      AdministratorRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<AdministratorRecord>> queryAdministratorRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      AdministratorRecord.collection,
      AdministratorRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<AdministratorRecord>> queryAdministratorRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Stream<List<T>> queryCollection<T>(
  ...
  return query.snapshots().handleError((err) {
    log('Error querying $collection: $err');
  }).map((s) => s.docs
      .map(
        (d) => safeGet(
          () => recordBuilder(d),
          (e) => log('Error serializing doc ${d.reference.path}:\n$e'),
        ),
      )
      .where((d) => d != null)
      .map((d) => d!)
